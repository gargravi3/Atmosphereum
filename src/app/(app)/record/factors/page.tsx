...
import { factors, CATEGORY_LABELS } from "@/lib/fixtures";
import { SectionHeader } from "@/components/ui/section-header";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { fmt, cn } from "@/lib/utils";

const SOURCES = ["all", "DEFRA 2023", "IGES", "USEEIO", "Climatiq", "HIGG", "Custom"] as const;
  ...
  return (
    <div className="px-8 py-10 space-y-8">
      ...
      <div className="grid grid-cols-4 gap-0 divide-x divide-rule border-y border-rule py-6">
        ...
        <div className="px-6">
          <div className="text-micro uppercase tracking-widest text-ink-muted mb-2">DEFRA 2023</div>
          <div className="display-number text-3xl">{factors.filter(f => f.source === "DEFRA 2023").length}</div>
        </div>
        <div className="px-6">
          <div className="text-micro uppercase tracking-widest text-ink-muted mb-2">USEEIO</div>
          <div className="display-number text-3xl">{factors.filter(f => f.source === "USEEIO").length}</div>
        </div>
        <div className="px-6">
          <div className="text-micro uppercase tracking-widest text-ink-muted mb-2">Custom</div>
          <div className="display-number text-3xl">0</div>
        </div>
      </div>
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-1 mb-3">
                  ...
                  <Badge variant="default">{CATEGORY_LABELS[f.category] ?? f.category}</Badge>
                </div>
              </div>
            </div>
