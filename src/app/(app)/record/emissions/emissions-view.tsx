...
import type { EmissionRecord } from "@/lib/db";

type FacilityMini = { id: string; name: string; geography: string };
type FactorMini = { id: string; name: string; source: string; version: string };
...
export function EmissionsView({
  ...
  }, [emissions, scope, facilityId, query]);
            ...
            <tbody>
              {filtered.slice(0, 60).map((e) => {
                ...
                return (
                  ...
                  >
                    <Td mono className="text-ink-muted">{e.id.slice(0, 8)}</Td>
                    <Td mono>{e.period}</Td>
                    <Td>
                      <span className={cn(
        ...
        <div className="sticky top-0 bg-paper border-b border-rule px-6 py-5 flex items-start justify-between gap-4">
          <div>
            <div className="text-micro uppercase tracking-[0.2em] text-ember mb-2">
              Emission record · {record.id.slice(0, 8)}
            </div>
