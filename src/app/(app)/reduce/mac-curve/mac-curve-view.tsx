...
export function MacCurveView({ opportunities }: { opportunities: OpportunityRow[] }) {
  ...
  const sorted = useMemo(
    ...
    [opportunities, capexLimit]
  );

  const totalAbate = sorted.reduce((a, o) => a + o.abatement_tco2e, 0);
  const maxMac = Math.max(...sorted.map((o) => o.mac_cost), 1);
  const minMac = Math.min(...sorted.map((o) => o.mac_cost), -1);

  let cumX = 0;
  const plotted = sorted.map((o) => {
    const x0 = cumX;
    cumX += o.abatement_tco2e;
    const width = totalAbate === 0 ? 0 : (o.abatement_tco2e / totalAbate) * 100;
    const startPct = totalAbate === 0 ? 0 : (x0 / totalAbate) * 100;
    const heightPct = (Math.abs(o.mac_cost) / Math.max(Math.abs(maxMac), Math.abs(minMac))) * 45;
  ...
  return (
    <div className="px-8 py-10 space-y-10">
      <SectionHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <Button variant={capexLimit === null ? "default" : "outline"} size="sm" onClick={() => setCapexLimit(null)}>
              Unconstrained
            </Button>
            <Button variant={capexLimit === 500_000 ? "default" : "outline"} size="sm" onClick={() => setCapexLimit(500_000)}>
              Capex ≤ £500k
            </Button>
            <Button variant={capexLimit === 100_000 ? "default" : "outline"} size="sm" onClick={() => setCapexLimit(100_000)}>
              Capex ≤ £100k
            </Button>
          </div>
        }
      ...
      <div className="grid grid-cols-4 gap-0 divide-x divide-rule border-y border-rule py-6">
        <div className="px-6">
          <div className="text-micro uppercase tracking-widest text-ink-muted mb-2">Total abatement</div>
          <div className="display-number text-3xl">
            {fmt.int(totalAbate)}<span className="text-sm text-ink-muted ml-1">tCO₂e</span>
          </div>
        </div>
        <div className="px-6">
          <div className="text-micro uppercase tracking-widest text-moss mb-2">Net-positive abatement</div>
          ...
          <div className="text-xs text-ink-muted font-mono mt-1">from {belowLine.length} initiatives</div>
        </div>
        <div className="px-6">
          <div className="text-micro uppercase tracking-widest text-moss mb-2">Annual savings</div>
          <div className="display-number text-3xl text-moss">+{fmt.gbpShort(belowLineSavings)}</div>
          <div className="text-xs text-ink-muted font-mono mt-1">if fully executed</div>
        </div>
        <div className="px-6">
          <div className="text-micro uppercase tracking-widest text-ink-muted mb-2">Paid abatement</div>
          <div className="display-number text-3xl text-ochre">
            {fmt.int(totalAbate - belowLineAbate)}
            <span className="text-sm text-ink-muted ml-1">tCO₂e</span>
          </div>
          <div className="text-xs text-ink-muted font-mono mt-1">from {aboveLine.length} initiatives</div>
        </div>
      </div>

      <section>
        <div className="text-micro uppercase tracking-[0.25em] text-ink-muted mb-3">§ Abatement cost curve</div>
        <div className="bg-paper-soft border border-rule p-8">
          <div className="relative" style={{ height: "360px" }}>
            <div className="absolute left-0 right-0 border-t border-ink-muted border-dashed" style={{ top: "50%" }} />
            <div className="absolute right-0 text-[10px] font-mono text-ink-muted px-2" style={{ top: "calc(50% - 10px)" }}>
              £0/tCO₂e
            </div>
              ...
              {plotted.map((p) => {
                ...
                return (
                  <div
                    ...
                    style={{ left: `${p.startPct}%`, width: `${p.width}%` }}
                  ...
                  >
                    ...
                    {isHover && (
                      <div className="absolute -top-2 left-1/2 -translate-x-1/2 -translate-y-full z-10 bg-ink text-paper text-xs p-3 whitespace-nowrap shadow-lift font-mono tabular">
                        <div className="font-sans font-medium mb-1 normal-case max-w-xs whitespace-normal">{p.opp.title}</div>
                        <div>
                          <span className="text-ink-muted">Abatement:</span> {fmt.int(p.opp.abatement_tco2e)} tCO₂e
                        </div>
                        <div>
                          <span className="text-ink-muted">MAC:</span>{" "}
                          <span className={isSaving ? "text-moss-soft" : "text-ember-soft"}>£{fmt.int(p.opp.mac_cost)}/t</span>
                        </div>
                        <div>
                          <span className="text-ink-muted">Savings/yr:</span> {fmt.gbpShort(p.opp.annual_savings_gbp)}
                        </div>
                      </div>
                    )}
                  </div>
