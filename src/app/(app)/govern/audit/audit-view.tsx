...
export function AuditView({ events }: { events: AuditEvent[] }) {
  ...
  const entities = useMemo(() => Array.from(new Set(events.map((e) => e.entity_type))).sort(), [events]);
  ...
  const groups = useMemo(() => {
    return filtered.reduce<Record<string, AuditEvent[]>>((acc, e) => {
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-baseline gap-2 text-sm mb-1 flex-wrap">
                            ...
                            <span className="text-ink-muted">{e.action.replace(/_/g, " ")}</span>
                            <Badge variant="outline">{e.entity_type}</Badge>
                            <span className="text-[10px] font-mono text-ink-muted">{e.entity_id.slice(0, 8)}</span>
                            <span className="ml-auto text-[10px] font-mono text-ink-muted">{time}</span>
                          </div>
                          {e.detail && <div className="text-xs text-ink-soft leading-relaxed mt-1">{e.detail}</div>}
                        </div>
                      </div>
                    </div>
                  </div>
